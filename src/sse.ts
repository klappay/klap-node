...
/**
 * Lower-level than `streamChargeEvents` below (which filters this down
 * to `event: charge` and unwraps `data`) — yields every named SSE event
 * on the stream (`event: <name>` + `data: <json>` pairs; a comment-only
 * heartbeat `: ping` line has neither, so it's silently skipped).
 */

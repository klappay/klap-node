...
- 68b08c7: Bump `@klappay/types` to `^5.1.1`: fixes a bundle-size regression in
...
- 53a41c9: Bump `@klappay/types` to `^5.1.0`: `Network` gains `tron` and `arc`
...
- 520e325: Add `charges.watchEvents(id, signal?)`: the same live SSE stream as `watch()`, without the `event: charge` filter, so `confirmation_progress` events can also be observed on the same connection. Exports the `isChargeEvent`/`isConfirmationProgressEvent` type guards (used internally by `waitForConfirmation()`) to discriminate the resulting union. `watch()` is unchanged.
